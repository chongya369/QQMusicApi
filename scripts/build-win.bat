...
chcp 65001 >nul
setlocal
REM QQMusicApi Windows 本地打包脚本: 产出 dist\qqmusic-web.exe
...
".venv-pack\Scripts\pyinstaller.exe" --noconfirm --clean --onefile --name qqmusic-web --paths . --hidden-import web.src.app --hidden-import web.src.routes --hidden-import web.src.modules --hidden-import uvicorn.logging --hidden-import uvicorn.loops.auto --hidden-import uvicorn.protocols.http.auto --hidden-import uvicorn.protocols.websockets.auto --hidden-import uvicorn.lifespan.on web\run.py || goto :error
...
echo.
echo 打包完成: %ROOT%\dist\qqmusic-web.exe
echo 使用方式: 将 qqmusic-web.exe 与 config.toml 放同一目录后运行
exit /b 0
