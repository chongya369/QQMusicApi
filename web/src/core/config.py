...
if _FROZEN:
    # PyInstaller 打包后,配置文件与 exe 同目录
    PROJECT_ROOT = Path(sys.executable).resolve().parent
else:
    PROJECT_ROOT = Path(__file__).resolve().parent.parent.parent.parent

# 配置文件目录: 开发时位于 PROJECT_ROOT/web/, 打包后位于 exe 同目录
CONFIG_DIR = PROJECT_ROOT if _FROZEN else PROJECT_ROOT / "web"
...
class Settings(BaseSettings):
    ...
    model_config = SettingsConfigDict(
        ...
        toml_file=str(CONFIG_DIR / "config.toml"),
